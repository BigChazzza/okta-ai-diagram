...
import type { CategoryMeta } from "@/lib/componentCategories";
...
export function CategorySection({
  ...
  const totalOnCanvas = components.reduce(
    (sum, c) => sum + (placedCounts[c.id] ?? 0),
    0,
  );
      ...
      <header className="flex items-center gap-1 px-3 py-2">
        ...
        >
          ...
          <span className="rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px] font-medium normal-case text-slate-600 dark:bg-slate-700 dark:text-slate-200">
            {totalOnCanvas}/{components.length}
          </span>
        </button>
        <button
          type="button"
        ...
        <div className="space-y-1.5 px-3 pb-3">
          ...
          {components.map((c) => (
            <DraggableItem
              key={c.id}
              component={c}
              placedCount={placedCounts[c.id] ?? 0}
            />
          ))}
        </div>
      )}
